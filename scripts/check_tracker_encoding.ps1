...
if ($content -match 'BLOCKER10K_FISH_ONLY_RAW_PROOF_2026_06_04') { Write-Host "PASS  BLOCKER10K build id found" } else { $errors += "FAIL  BLOCKER10K build id missing" }
...
if ($content -match 'enableTargetedItemDiagnostics = false') { Write-Host "PASS  enableTargetedItemDiagnostics default false" } else { $errors += "FAIL  enableTargetedItemDiagnostics must be false" }
...
if ($content -match 'PLAYER_DATA_ONLY enabled=') { Write-Host "PASS  PLAYER_DATA_ONLY log found" } else { $errors += "FAIL  PLAYER_DATA_ONLY log missing" }
