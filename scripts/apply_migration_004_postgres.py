#!/usr/bin/env python3
"""Apply migration 004 (license_key_executions) via direct Postgres connection.
...
Usage:
    python scripts/apply_migration_004_postgres.py
...
def main() -> int:
    ...
    sql_path = PROJECT_ROOT / "supabase" / "migrations" / "004_key_executions.sql"
    ...
    attempts = [
        # Direct DB connection with DB password
        {"host": f"db.{ref}.supabase.co", "port": 5432, "user": "postgres", "password": pwd or key},
        # Supabase pooler (transaction mode, requires postgres.{ref} user format)
