...
const LS_KEY        = 'fishit_tracked_users';
  ...
  const RARITY_ORDER = {
    Secret: 900, Forgotten: 800, Mythic: 600, Legendary: 500, Epic: 400,
  ...
  function patchFishCardDom(card, item, opts) {
    ...
    patchCardImage(img, item, imgSrc, true, !!(opts && opts.eagerImage));
  ...
  function isAccountPresent(entry) {
    const st = entryUploadStatus(entry);
    if (st && st.statusColor) return st.statusColor === 'green' || st.statusColor === 'yellow';
  ...
  function entryInventoryRedSince(entry) {
    ...
    const ts = entryInventoryUploadTimestamp(entry);
    const interval = (data && Number(data.intervalSeconds) > 0) ? Number(data.intervalSeconds) : DEFAULT_UPLOAD_INTERVAL_SEC;
    const staleAfter = Number(data && data.inventoryStaleAfterSeconds) > 0
      ? Number(data.inventoryStaleAfterSeconds)
      : inventoryUploadStaleAfterSeconds(interval);
    if (ts) {
      const deadlineMs = new Date(ts).getTime() + staleAfter * 1000;
  ...
  function touchCaughtActivityState(entry, stats, nowIso) {
    if (!entry) return;
    const current = normalizeTotalCaughtValue(stats);
    if (current == null) return;
    const now = nowIso || new Date().toISOString();
    const prev = entry._lastTotalCaughtValue;
    if (prev == null || current > prev) {
      entry._lastTotalCaughtValue = current;
      entry._lastCaughtIncreaseAt = now;
      return;
    }
    if (entry._lastTotalCaughtValue == null) entry._lastTotalCaughtValue = current;
  ...
  function formatCaughtActivitySub(entry) {
    if (!entry || !entry._lastCaughtIncreaseAt) return '';
    const label = formatMinimalSyncDuration(entry._lastCaughtIncreaseAt);
    if (!label) return '';
    return `(${label} ago)`;
  }
  ...
  function entryConnectionFreshness(entry) {
    const st = entryUploadStatus(entry);
    if (st && st.statusColor) return statusColorToFreshness(st.statusColor);
    const data = entry && entry.lastData;
    if (data && data.statusColor) return statusColorToFreshness(data.statusColor);
    if (data && data.currentStatus) return statusColorToFreshness(data.currentStatus);
    return 'dead';
  }
  ...
  function patchAccountStatusDom(root, entry) {
    ...
    const syncEl = root.querySelector('[data-table-status-sync]');
    const fresh = entryConnectionFreshness(entry);
    if (statusEl) {
      statusEl.classList.remove('live', 'stale', 'dead');
      statusEl.classList.add(fresh === 'live' ? 'live' : fresh === 'stale' ? 'stale' : 'dead');
      const label = fresh === 'live' ? 'Online' : fresh === 'stale' ? 'Delayed' : 'Offline';
      statusEl.setAttribute('title', label);
      statusEl.setAttribute('aria-label', label);
    }
    if (syncEl) {
      const syncLabel = formatPresenceStatusText(entry);
      syncEl.textContent = syncLabel || '';
    }
  }
  function patchAccountStatsSyncSub(root, entry) {
    if (!root || !entry) return;
    const subEl = root.querySelector('[data-stats-sync-sub]');
    if (!subEl) return;
  ...
  function buildAccountStatusHtml(entry) {
    ...
    const dotClass = freshness === 'live' ? 'live' : freshness === 'stale' ? 'stale' : 'dead';
    const syncText = formatPresenceStatusText(entry);
    return `<span class="accounts-status"><span class="status-dot ${dotClass}" data-table-status-dot aria-hidden="true"></span>${syncText ? `<span class="accounts-status__text" data-table-status-sync>${escHtml(syncText)}</span>` : ''}</span>`;
  }
  ...
  function patchInventoryUploadIndicatorDom(root, entry) {
    ...
    const textEl = root.querySelector('[data-inventory-upload-text]');
  ...
  function patchBulkStoneGrid(container, items, opts) {
    ...
    if (!items || items.length === 0) {
      if (!container.querySelector('.inventory-search-empty')) {
        container.innerHTML = '<div class="inventory-search-empty">No inventory items found</div>';
      }
      return;
    }
    let grid = container.querySelector('.stones-grid, .stone-grid');
    if (!grid) {
      container.innerHTML = '<div class="items-grid inventory-grid stones-grid stone-grid"></div>';
      grid = container.querySelector('.stones-grid');
    }
    container.querySelectorAll('.inventory-search-empty, .card-empty').forEach((el) => el.remove());
    ...
    items.forEach((item, idx) => {
      const key = bulkCardKey(item);
      const itemOpts = typeof buildOpts === 'function' ? buildOpts(item) : (opts.cardOpts || {});
      nextKeys.add(key);
  ...
  function renderBulkInventory(showCategory) {
    ...
    if (showCategory === 'stone') {
      ...
      bulkBodyEl.querySelectorAll('.inventory-search-empty').forEach((el) => el.remove());
      patchBulkStoneGrid(host, stones, {
  ...
  function patchCardInventory(cardBody, fishList, stoneList) {
    ...
    if (showStones) patchStonesGrid(stoneHost, stoneList || [], INDIVIDUAL_BACKPACK_CARD_OPTS);
...
<div class="card-body" data-card-body></div>`;
  ...
  let activeInventorySection = 'dashboard';
  ...
  function sortDashboardFishCards(cards) {
    const rank = (rarity) => RARITY_ORDER[String(rarity || 'Unknown').trim()] ?? RARITY_ORDER.Unknown;
    return [...(Array.isArray(cards) ? cards : [])].sort((a, b) => {
      const rarityDiff = rank(b.rarity) - rank(a.rarity);
      if (rarityDiff) return rarityDiff;
      return (Number(b.amount) || Number(b.count) || 0) - (Number(a.amount) || Number(a.count) || 0);
    });
  }
  ...
  function applyDashboardData(data) {
    ...
    if (cu) {
      if (secretEl) {
        if (!failed) cu.set(secretEl, { to: secretVal, format: 'integer', duration: 1200 });
        else secretEl.textContent = '—';
      }
      if (forgottenEl) {
        if (!failed) cu.set(forgottenEl, { to: forgottenVal, format: 'integer', duration: 1200 });
    ...
    renderDashboardChart((data && data.dailyCaught) || [], { failed });
  ...
  async function loadDashboardData(force) {
    ...
    try {
      ...
      const raw = await res.text();
      notePerfFetch(raw);
      data = res.ok ? JSON.parse(raw) : null;
      if (!data || !data.ok) {
      ...
      applyDashboardData({
        ...
        statsState: 'error',
        available: false,
        emptyReason: (data && data.error) || 'dashboard_unavailable',
  ...
  function initDashboardSectionFromQuery() {
    ...
    if (section === 'accounts' || section === 'tracker' || section === 'live') {
  ...
  function initInventoryUi() {
    ...
    safeBind('restore sessions', () => {
      restoreTrackedAccountsFromServer()
        .then(() => {
          if (activeInventorySection === 'accounts') {
            startLiveTrackerPolling();
          } else {
    ...
    safeBind('sync age tick', () => {
      setInterval(tickAllCardSyncStatus, SYNC_TICK_MS);
    });
    safeBind('visibility refetch', () => {
      document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible' && activeInventorySection === 'accounts') {
          refetchAllAccountStatus(true);
        }
      });
      window.addEventListener('focus', () => {
        if (activeInventorySection === 'accounts') refetchAllAccountStatus(true);
      });
      window.addEventListener('pageshow', (ev) => {
        if (ev.persisted && activeInventorySection === 'accounts') refetchAllAccountStatus(true);
      });
    });
