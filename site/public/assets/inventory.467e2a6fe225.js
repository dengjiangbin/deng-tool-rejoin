    ...
    for (const { owner, row } of instances) {
      ...
      if (list && list.length) {
        for (const inst of list) {
          ...
          const weightVal = (inst && (inst.weightKg != null ? inst.weightKg
            : (inst.metadataWeightKg != null ? inst.metadataWeightKg : inst.weight)));
          const weight = ftWeightKgText(weightVal);
          for (let i = 0; i < amount; i += 1) {
            if (!pushCard({ owner, mutation, name, weight, imgSrc, rarity })) return ftSortFishCards(cards);
          }
        }
      } else {
        ...
        for (let i = 0; i < amount; i += 1) {
          if (!pushCard({ owner, mutation, name: baseName, weight, imgSrc, rarity })) return ftSortFishCards(cards);
        }
      }
    }
