...
const STONE_DISPLAY_NAMES = {"Normal":"Normal Enchant Stone","Double":"Transcended Stone","Evolved":"Evolved Enchant Stone","Eggy":"Eggy Enchant Stone","Runic":"Runic Enchant Stone"};
  ...
  function formatPresenceStatusText(entry) {
    ...
    const data = entry && entry.lastData;
    const ts = data && (data.lastAccountSeenAt || data.lastHeartbeatAt || data.lastSeenAt);
  ...
  function getPublicTotemItems(data) {
    ...
    return sortInventoryTotems(items);
  }
  ...
  function patchTotemsSubgrid(host, items, opts) {
    ...
    const titleText = `Totem (${formatQuantity(totemTotal)})`;
    if (title) title.textContent = titleText;
  ...
  function patchItemGrid(container, stoneItems, totemItems, opts) {
    ...
    let mainTitle = container.querySelector('.items-section__title');
    if (!mainTitle) {
      container.innerHTML = [
        '<div class="items-section__title">Item Grid</div>',
      ...
      mainTitle = container.querySelector('.items-section__title');
    }
    if (mainTitle && mainTitle.textContent !== 'Item Grid') mainTitle.textContent = 'Item Grid';
