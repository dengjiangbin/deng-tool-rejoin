...
const LS_KEY        = 'fishit_tracked_users';
  ...
  const EN_DASH = '\u2013';
  const EM_DASH = '\u2014';
  const TRACKER_BUILD_MARKER = 'TRACKER_AUTH_UPLOAD_STALENESS_FIX_2026_06_18';
  try {
  ...
  function formatAgeAgo(ms) {
    ...
    let n;
    let unit;
    if (totalSecs < 60) { n = Math.max(1, totalSecs); unit = 's'; }
    else if (totalSecs < 3600) { n = Math.floor(totalSecs / 60); unit = 'm'; }
    else if (totalSecs < 86400) { n = Math.floor(totalSecs / 3600); unit = 'h'; }
    else { n = Math.floor(totalSecs / 86400); unit = 'd'; }
    return `${n}${unit} ago`;
  }
  ...
  function seedSectionBaseFromBackendAge(entry, field, ageSecs) {
    ...
    entry[field] = (Number.isFinite(n) && n >= 0)
      ? Date.now() - Math.round(n * 1000)
      : Date.now();
  }
  function maybeResetSectionTimers(entry) {
    if (!entry) return;
    const data = entry.lastData;
    if (hasRenderableTrackerData(data)) {
    ...
    if (payloadHasLeaderstats(data)) {
      const sig = buildLeaderstatsSignature(entry);
      if (sig !== entry._leaderstatsDisplaySig) {
        const firstObservation = entry._leaderstatsDisplaySig === undefined;
        entry._leaderstatsDisplaySig = sig;
        if (firstObservation) seedSectionBaseFromBackendAge(entry, '_leaderstatsFrontendRefreshAt', backendStatsAgeSeconds(entry));
        else markEntryLeaderstatsRefreshed(entry);
      }
    }
    if (entryHasInventoryRows(data)) {
      const sig = buildInventorySignature(data);
      if (sig !== entry._inventoryDisplaySig) {
        const firstObservation = entry._inventoryDisplaySig === undefined;
        entry._inventoryDisplaySig = sig;
        if (firstObservation) seedSectionBaseFromBackendAge(entry, '_inventoryFrontendRefreshAt', backendInventoryAgeSeconds(entry));
  ...
  function formatPresenceStatusText(entry) {
    return formatAgeAgoSeconds(backendPresenceAgeSeconds(entry));
  }
  function formatStatsUploadDurationText(entry) {
    return formatAgeAgoSeconds(backendStatsAgeSeconds(entry));
  }
  ...
  function seedTimersFromBackend(entry) {
    ...
    const now = Date.now();
    const seedIfUnset = (field, ageSecs) => {
      if (entry[field] != null) return;
      const n = Number(ageSecs);
      if (!Number.isFinite(n) || n < 0) return;
      entry[field] = now - Math.round(n * 1000);
    };
    seedIfUnset('_frontendRefreshAt', backendPresenceAgeSeconds(entry));
    seedIfUnset('_leaderstatsFrontendRefreshAt', backendStatsAgeSeconds(entry));
  ...
  function formatEntrySyncStatusText(entry) {
    if (!entry) return '';
    return formatAgeAgoSeconds(backendInventoryAgeSeconds(entry));
  }
  ...
  function patchInventoryUploadIndicatorDom(root, entry) {
    ...
    if (dotEl) {
      ...
      if (!dotEl.classList.contains(want)) {
        ...
        dotEl.classList.add(want);
      }
    }
    if (textEl && textEl.textContent !== (label || '')) textEl.textContent = label || '';
    if (wrapEl && wrapEl.matches('[data-inventory-upload-indicator]')) {
      const isLive = !!fresh;
      if (wrapEl.classList.contains('is-live') !== isLive) wrapEl.classList.toggle('is-live', isLive);
      if (wrapEl.classList.contains('is-stale') === isLive) wrapEl.classList.toggle('is-stale', !isLive);
      const title = isLive ? 'Fish and stone upload fresh' : 'Fish and stone upload stale';
  ...
  function updateInventoryUploadIndicator(preferredEntry) {
    ...
    if (bulkIndicator) {
      ...
      setInventoryIndicatorHidden(bulkIndicator, !scoped);
      if (scoped) patchInventoryUploadIndicatorDom(bulkIndicator, entry);
    }
    if (accountViewMode === 'account' && activeAccountKey) {
      const active = trackers.get(activeAccountKey);
