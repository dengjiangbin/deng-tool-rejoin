...
const LS_KEY        = 'fishit_tracked_users';
  ...
  const TRUSTED_PLAYERSTATS_BUILD_MARKS = ['INVENTORY_SNAPSHOT_NIL_FIX_METADATA_SCAN', 'INSTANCE_MUTATION_WEIGHT_DETAIL', 'METADATA_PROBE_DEEP_SCAN', 'BLOCKER10ZT5', 'BLOCKER10ZT4', 'BLOCKER10ZT3', 'BLOCKER10ZW'];
    ...
    for (const { owner, row } of instances) {
      ...
      if (list && list.length) {
        for (const inst of list) {
          ...
          const mutRaw = (inst && (inst.mutationName || inst.mutation || inst.metadataMutation)) || '';
          const mutation = String(mutRaw).trim();
