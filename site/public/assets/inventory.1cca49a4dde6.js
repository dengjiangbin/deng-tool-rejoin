...
const LS_KEY        = 'fishit_tracked_users';
  ...
  function isTrackerOnline(entry) {
    ...
    if (data && (data.connectionLive === true || data.isOnline === true)) return true;
    const ts = entry.lastSyncAt || bestSyncTimestamp(data);
    if (syncFreshnessFromTimestamp(ts) === 'live') return true;
    if (entry.lastPollOkAt && syncAgeSeconds(entry.lastPollOkAt) <= SYNC_LIVE_MAX_SEC) {
  ...
  function formatTableSyncStatusText(entry) {
    if (!entry) return 'no sync';
    return formatSyncDurationLabel(entryDisplaySyncTimestamp(entry));
  }
  function formatEntrySyncStatusText(entry) {
    if (!entry) return 'no sync';
    const duration = formatTableSyncStatusText(entry);
    const name = formatUsernameForDisplay(entry.displayName, { hideUsernames });
  ...
  function entryConnectionFreshness(entry) {
    ...
    if (entry.lastPollOkAt && syncAgeSeconds(entry.lastPollOkAt) <= SYNC_LIVE_MAX_SEC) return 'live';
    const serverTs = entry.lastServerSyncAt || bestSyncTimestamp(entry.lastData);
    if (serverTs && syncAgeSeconds(serverTs) <= SYNC_LIVE_MAX_SEC) return 'live';
    if (isTrackerOnline(entry)) return 'live';
    return 'dead';
  }
  ...
  <div class="accounts-mobile-card__grid accounts-mobile-card__grid--stats">
    <div class="accounts-mobile-card__row"><span class="accounts-mobile-card__row-label">Coin</span><span class="accounts-mobile-card__row-value${displayCoinsStat(stats) === 'ΓÇö' ? ' is-muted' : ''}">${escHtml(displayCoinsStat(stats))}</span></div>
    <div class="accounts-mobile-card__row"><span class="accounts-mobile-card__row-label">Caught</span><span class="accounts-mobile-card__row-value${displayTotalCaughtStat(stats) === 'ΓÇö' ? ' is-muted' : ''}">${escHtml(displayTotalCaughtStat(stats))}</span></div>
    <div class="accounts-mobile-card__row"><span class="accounts-mobile-card__row-label">Rare</span><span class="accounts-mobile-card__row-value${displayRarestFishStat(stats) === 'ΓÇö' ? ' is-muted' : ''}">${escHtml(displayRarestFishStat(stats))}</span></div>
  </div>
</article>`;
  }
  ...
  <td class="accounts-table__stat col-coins${displayCoinsStat(stats) === 'ΓÇö' ? ' is-muted' : ''}">${escHtml(displayCoinsStat(stats))}</td>
  <td class="accounts-table__stat col-caught${displayTotalCaughtStat(stats) === 'ΓÇö' ? ' is-muted' : ''}">${escHtml(displayTotalCaughtStat(stats))}</td>
  <td class="accounts-table__stat col-rare${displayRarestFishStat(stats) === 'ΓÇö' ? ' is-muted' : ''}">${escHtml(displayRarestFishStat(stats))}</td>
  ...
  function refreshEntrySyncDisplay(entry) {
    ...
    if (dotEl) {
      ...
      dotEl.classList.add(fresh === 'live' ? 'live' : 'dead');
    }
    ...
    if (DEBUG_INVENTORY) {
      window.__fishitDebugProof = {
        ...
        trackerLuaTouchProof: {
          ...
          liveDistRelativePath: 'dist/tracker.lua',
        },
        statusNoZeroProof: {
