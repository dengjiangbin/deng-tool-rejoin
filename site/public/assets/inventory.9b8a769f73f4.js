...
const LS_KEY        = 'fishit_tracked_users';
  ...
  function ftFishInstanceCards(instances) {
    ...
    for (const { owner, row } of instances) {
      ...
      for (let i = 0; i < amount; i += 1) {
        cards.push({ owner, mutation, name, weight });
        if (cards.length >= FT_MAX_INSTANCE_CARDS) return ftSortFishCards(cards);
      }
    }
  ...
  function renderFishInstanceCard(card) {
    ...
    const weight = card.weight ? `<div class="ft-inst-card__weight">Weight: ${escHtml(card.weight)}</div>` : '';
    return `<div class="ft-inst-card">${mut}<div class="ft-inst-card__name">${escHtml(card.name)}</div><div class="ft-inst-card__owner">${escHtml(card.owner || '-')}</div>${weight}</div>`;
  }
    ...
    if (k === 'fish') {
      ...
      if (bodyEl) {
        ...
        bodyEl.innerHTML = cards.length
          ? cards.map(renderFishInstanceCard).join('')
