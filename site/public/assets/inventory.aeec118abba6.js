...
const LS_KEY        = 'fishit_tracked_users';
  ...
  let trackedAccountsCache = [];
  let removeMenuIgnoreCloseUntil = 0;
  let removeMenuDocumentBound = false;
  let bulkSearchQuery  = '';
  ...
  function serverAccountDisplayName(acct) {
    if (!acct) return '';
    return acct.robloxUsername || acct.displayName || acct.username || '';
  }
  function normalizeCanonicalTrackedAccount(acct) {
    if (!acct || typeof acct !== 'object') return null;
    const key = String(acct.key || acct.robloxUsernameKey || acct.roblox_username_key || '').toLowerCase();
    const username = serverAccountDisplayName(acct) || (key || '');
    if (!key || !username) return null;
    return { key, username, displayName: username };
  }
  function updateTrackedAccountsCache(accounts) {
    if (!Array.isArray(accounts)) return;
    trackedAccountsCache = accounts
      .map((acct) => normalizeCanonicalTrackedAccount(acct))
      .filter(Boolean);
  }
  function getCanonicalTrackedAccountsFromMap() {
    const rows = [];
    trackers.forEach((entry, key) => {
      const username = (entry && entry.displayName) || key;
      if (!key || !username) return;
      rows.push({ key, username, displayName: username });
    });
    rows.sort((a, b) => a.username.localeCompare(b.username));
    return rows;
  }
  function getCanonicalTrackedAccounts() {
    const fromMap = getCanonicalTrackedAccountsFromMap();
    if (fromMap.length) return fromMap;
    return trackedAccountsCache.slice().sort((a, b) => a.username.localeCompare(b.username));
  }
  async function fetchCanonicalTrackedAccounts() {
    const res = await inventoryAccountsRequest('', { method: 'GET' });
    if (res.ok && Array.isArray(res.data && res.data.accounts)) {
      updateTrackedAccountsCache(res.data.accounts);
      syncTrackersWithServerAccounts(res.data.accounts, { skipCacheUpdate: true });
      return getCanonicalTrackedAccounts();
    }
    return getCanonicalTrackedAccounts();
  }
  ...
  function syncTrackersWithServerAccounts(accounts, opts) {
    const list = Array.isArray(accounts) ? accounts : [];
    if (!opts || opts.skipCacheUpdate !== true) updateTrackedAccountsCache(list);
    list.forEach((acct) => {
  ...
  function patchStoneCardDom(card, item, opts) {
    ...
    const entry = entryKey && trackers.get(entryKey);
    if (entry && cardKeyVal) {
      const prevList = kind === 'totem' ? (entry.lastTotemList || []) : (entry.lastStoneList || []);
      const prev = prevList.find((row) => {
        const rowKey = kind === 'totem' ? totemCardKey(row) : stoneCardKey(row);
        return rowKey === cardKeyVal;
      });
      if (prev) mergedItem = preferFishCardImageFields(prev, item);
    }
    const title = kind === 'totem'
      ? String(mergedItem.name || mergedItem.displayName || 'Totem')
      : stoneDisplayName(mergedItem);
    const imgSrc = itemImageSrc(mergedItem);
    const icon = card.querySelector('.ft-card-icon');
    if (imgSrc) {
      let img = icon && icon.querySelector('img');
      if (!img && icon) {
      ...
      patchCardImage(img, mergedItem, imgSrc, false, !!(opts && opts.eagerImage));
    } else if (icon) {
      const fallback = kind === 'totem' ? '\u{1F9FF}' : '\u{1F48E}';
      if (icon.textContent !== fallback) icon.textContent = fallback;
    }
    setElementTextIfChanged(card.querySelector('.ft-card-name'), title, title);
    patchHtmlIfChanged(card.querySelector('.ft-card-stats'), buildStoneStatsHtml(mergedItem, opts));
  }
  function escHtml(s) {
    return String(s)
      .replace(/&/g, '\u0026amp;')
  ...
  const DEFAULT_UPLOAD_INTERVAL_SEC = 10;
  ...
  function mergeUploadStatusOntoEntry(entry, statusRow, serverNow) {
    ...
    entry._uploadStatusFetchedAtMs = serverNow
      ...
      : Date.now();
    touchFreshUploadState(entry, statusRow);
  }
  function touchFreshUploadState(entry, data) {
    if (!entry || !data) return;
    const uploadSeq = data.uploadSequence != null
      ? Number(data.uploadSequence)
      : (data.uploadSeq != null ? Number(data.uploadSeq) : null);
    const freshAt = data.lastSuccessfulUploadAt
      || data.leaderstatsUploadedAt
      || data.lastRequiredUploadAt
      || data.serverReceivedAt
      || null;
    const prevSeq = entry._lastSeenUploadSeq;
    const prevFreshAt = entry._lastSeenSuccessfulUploadAt;
    const seqChanged = uploadSeq != null && Number.isFinite(uploadSeq) && uploadSeq !== prevSeq;
    const freshAtChanged = !!(freshAt && freshAt !== prevFreshAt);
    if (seqChanged || freshAtChanged) {
      entry._lastFreshUploadPulseAt = Date.now();
      if (uploadSeq != null && Number.isFinite(uploadSeq)) entry._lastSeenUploadSeq = uploadSeq;
      if (freshAt) entry._lastSeenSuccessfulUploadAt = freshAt;
    }
    if (data.sameValuesFreshSync === true) entry._sameValuesFreshSync = true;
    else if (data.playerStatsChanged === true) entry._sameValuesFreshSync = false;
  }
  function liveSecondsSinceLastSuccess(entry) {
    const st = entryUploadStatus(entry);
    if (!st || st.secondsSinceLastSuccess == null) return null;
    if (!entry._uploadStatusFetchedAtMs) return st.secondsSinceLastSuccess;
    const drift = Math.floor((Date.now() - entry._uploadStatusFetchedAtMs) / 1000);
    return st.secondsSinceLastSuccess + Math.max(0, drift);
  }
  ...
  function statsSnapshotReady(data) {
    ...
    if (data.leaderstatsUploadOk === true && data.playerStats) return true;
    if (data.lastValidLeaderstats && typeof data.lastValidLeaderstats === 'object') return true;
    if (data.liveAccountStats && !data.liveAccountStats.emptyReason
      && (data.lastValidLeaderstats || data.leaderstatsUploadOk === true)) {
      return true;
    }
    return false;
  }
  ...
  function isEntryStatusGreen(entry) {
    return isAccountPresent(entry);
  }
  function entryStatsUploadTimestamp(entry) {
    if (!entry) return null;
    const data = entry.lastData;
    const stats = getEntryPlayerStats(entry);
    return (data && (data.leaderstatsUploadedAt || data.lastStatsUploadAt || data.playerStatsUpdatedAt))
      || (stats && stats.statsAt)
      || null;
  }
  ...
  function isStatsUploadFresh(entry) {
    const data = entry && entry.lastData;
    const st = entryUploadStatus(entry);
    if (st && typeof st.statsUploadFresh === 'boolean') return st.statsUploadFresh;
    if (data && typeof data.statsUploadFresh === 'boolean') return data.statsUploadFresh;
    if (!data || data.leaderstatsUploadOk !== true) return false;
    const ts = data.leaderstatsUploadedAt || data.lastStatsUploadAt || data.playerStatsUpdatedAt;
    if (!ts) return false;
    const interval = Number(data.uploadIntervalSeconds || data.intervalSeconds) > 0
      ? Number(data.uploadIntervalSeconds || data.intervalSeconds)
      : DEFAULT_UPLOAD_INTERVAL_SEC;
    const age = syncAgeSeconds(ts);
    if (age == null) return false;
    return age <= interval + inventoryUploadGraceSeconds(interval);
  }
  ...
  function entryStatsHasValues(stats) {
    if (!stats || typeof stats !== 'object') return false;
    return stats.coins != null || stats.totalCaught != null
  ...
  function isTrustedPlayerStatsBuild(build) {
    ...
    if (!s) return true;
    if (s.includes('UPLOAD_COMPACT_FAST_PATH')) return true;
  ...
  function isTrustedPlayerStats(stats) {
    if (!stats || typeof stats !== 'object') return false;
    if (stats.__fromApi === true) return entryStatsHasValues(stats);
    const build = stats.build || '';
    const source = stats.source || '';
    if (entryStatsHasValues(stats) && isTrustedPlayerStatsSource(source)) return true;
    if (!isTrustedPlayerStatsBuild(build)) return false;
    return isTrustedPlayerStatsSource(source);
  }
  function displayableEntryPlayerStats(stats) {
    if (!stats || typeof stats !== 'object') return null;
    if (stats.__fromApi === true) {
      return entryStatsHasValues(stats) ? stats : null;
    }
    if (entryStatsHasValues(stats) && isTrustedPlayerStatsSource(stats.source)) return stats;
  ...
  function extractPlayerStatsFromPayload(data) {
    ...
    if (data.playerStats && typeof data.playerStats === 'object') {
      const fromCurrent = normalizePollPlayerStats({ ...data.playerStats, __fromApi: true });
      if (fromCurrent) return fromCurrent;
    }
    if (data.lastValidLeaderstats && typeof data.lastValidLeaderstats === 'object') {
      const fromLast = normalizePollPlayerStats({ ...data.lastValidLeaderstats, __fromApi: true });
      if (fromLast) return fromLast;
    }
    if (data.lastValidPlayerStats && typeof data.lastValidPlayerStats === 'object') {
      const fromAlias = normalizePollPlayerStats({ ...data.lastValidPlayerStats, __fromApi: true });
      if (fromAlias) return fromAlias;
    }
    if (data.liveAccountStats && entryStatsHasValues(data.liveAccountStats)) {
  ...
  function buildLiveSnapshotFromPayload(entry, data, pollAt) {
    ...
    const extracted = extractPlayerStatsFromPayload(data);
    const prevStats = (entry && entry.liveSnapshot && entry.liveSnapshot.playerStats)
      || (entry && entry.playerStats)
      || null;
    const playerStats = extracted || (entryStatsHasValues(prevStats) ? prevStats : null);
    return {
      pollAt,
  ...
  function entryPresenceTimestamp(entry) {
    const st = entryUploadStatus(entry);
    if (st && st.lastSuccessfulUploadAt) return st.lastSuccessfulUploadAt;
    const data = entry && entry.lastData;
    return data && data.lastSuccessfulUploadAt ? data.lastSuccessfulUploadAt : null;
  }
  ...
  function formatPresenceStatusText(entry) {
    const secs = liveSecondsSinceLastSuccess(entry);
    if (secs != null) return formatPresenceDurationLabel(secs);
    const data = entry && entry.lastData;
    const ts = data && (data.lastSuccessfulUploadAt || data.lastRequiredUploadAt
      || data.leaderstatsUploadedAt || data.lastAccountSeenAt || data.lastHeartbeatAt || data.lastSeenAt);
    const label = formatPresenceDurationLabel(syncAgeSeconds(ts));
    return label || '1s';
  }
  function formatCaughtActivitySub(entry) {
    const st = entryUploadStatus(entry);
    const data = entry && entry.lastData;
    const sameFresh = (st && st.sameValuesFreshSync === true)
      || (data && data.sameValuesFreshSync === true)
      || entry._sameValuesFreshSync === true;
    if (sameFresh) return 'No stat change';
    const ts = entryStatsChangeAt(entry);
    if (!ts) return '';
    const label = formatMinimalSyncDuration(ts);
    if (!label) return '';
    return `(${label} ago)`;
  }
  ...
  function formatEntrySyncStatusText(entry) {
    ...
    const ts = fresh ? entryInventoryUploadTimestamp(entry) : entryInventoryRedSince(entry);
    return formatMinimalSyncDuration(ts);
  }
  ...
  function entryConnectionFreshness(entry) {
    const st = entryUploadStatus(entry);
    const present = (st && typeof st.accountPresenceLive === 'boolean')
      ? st.accountPresenceLive
      : isAccountPresent(entry);
    return present ? 'live' : 'dead';
  }
  ...
  function patchAccountStatusDom(root, entry) {
    ...
    if (syncEl) {
      syncEl.textContent = formatPresenceStatusText(entry);
      const pulseAt = entry._lastFreshUploadPulseAt;
      if (pulseAt && (Date.now() - pulseAt) < 1200) {
        syncEl.classList.add('is-fresh-pulse');
      } else {
        syncEl.classList.remove('is-fresh-pulse');
      }
    }
  }
  ...
  function patchAccountStatsSyncSub(root, entry) {
    ...
    subEl.classList.remove('is-stale');
    const pulseAt = entry._lastFreshUploadPulseAt;
    if (pulseAt && (Date.now() - pulseAt) < 1200) {
      subEl.classList.add('is-fresh-pulse');
    } else {
      subEl.classList.remove('is-fresh-pulse');
    }
  }
  ...
  function applyAccountStatusStatsToEntry(entry, st) {
    if (!entry || !st) return;
    const merged = Object.assign({}, entry.lastData || {}, st);
    const normalized = extractPlayerStatsFromPayload(merged);
  ...
  function applyInventoryPollPayload(entry, key, data) {
    ...
    if (data && (data.statusColor || data.currentStatus)) {
      mergeUploadStatusOntoEntry(entry, {
        ...
        uploadSequence: data.uploadSequence != null ? data.uploadSequence : data.uploadSeq,
        syncSequence: data.syncSequence != null ? data.syncSequence : data.uploadSeq,
        lastRequiredUploadAt: data.lastRequiredUploadAt || data.leaderstatsUploadedAt || null,
        requiredOk: typeof data.requiredOk === 'boolean' ? data.requiredOk : undefined,
        playerStatsChanged: typeof data.playerStatsChanged === 'boolean' ? data.playerStatsChanged : undefined,
        inventoryChanged: typeof data.inventoryChanged === 'boolean' ? data.inventoryChanged : undefined,
        sameValuesFreshSync: typeof data.sameValuesFreshSync === 'boolean' ? data.sameValuesFreshSync : undefined,
        runId: data.runId,
    ...
    touchFreshUploadState(entry, data);
  ...
  function bulkGroupKey(category, item) {
    ...
    if (cat === 'totem') {
      const stable = String(item?.uuid || item?.itemId || name).toLowerCase();
      return `${cat}:${name.toLowerCase()}:${stable}`;
    }
  ...
  function mergeBulkItem(existing, item, username, category) {
    ...
    const merged = preferFishCardImageFields({
      ...existing,
  ...
  function renderRemoveAccountMenu(accounts) {
    ...
    removeMenuEl.innerHTML = list.map((acct) => {
      const key = acct.key || String(acct.username || '').toLowerCase();
      const name = acct.displayName || acct.username || key;
  ...
  function updateRemoveMenu() {
    renderRemoveAccountMenu(getCanonicalTrackedAccounts());
  }
  ...
  async function openRemoveMenu() {
    let accounts = getCanonicalTrackedAccounts();
    if (!accounts.length) {
      accounts = await fetchCanonicalTrackedAccounts();
    }
    renderRemoveAccountMenu(accounts);
    if (!removeMenuEl || !removeMenuBtn) return;
    removeMenuEl.hidden = false;
    removeMenuBtn.setAttribute('aria-expanded', 'true');
  }
  async function toggleRemoveMenu() {
    if (!removeMenuEl || !removeMenuBtn) return;
    if (!removeMenuEl.hidden) {
      closeRemoveMenu();
      return;
    }
    removeMenuIgnoreCloseUntil = Date.now() + 320;
    await openRemoveMenu();
  }
  ...
  function aggregateTotemItemsForDisplay(items) {
    ...
    for (const item of items) {
      ...
      if (prev) {
        ...
        map.set(key, preferFishCardImageFields(prev, item));
      } else {
  ...
  function patchStonesSubgrid(host, items, opts) {
    ...
    items.forEach((item, idx) => {
      ...
      if (!card) {
        card = buildStoneCardElement(item, opts);
        placeGridCardAtIndex(grid, card, idx);
      } else {
        patchStoneCardElement(card, item, opts);
        placeGridCardAtIndex(grid, card, idx);
      }
    });
  ...
  function patchTotemsSubgrid(host, items, opts) {
    ...
    items.forEach((item, idx) => {
      ...
      let card = existing.get(key);
      if (!card) {
        card = buildTotemCardElement(item, opts);
        placeGridCardAtIndex(grid, card, idx);
      } else {
        patchTotemCardElement(card, item, opts);
        placeGridCardAtIndex(grid, card, idx);
      }
    });
    existing.forEach((el, key) => { if (!nextKeys.has(key)) el.remove(); });
  ...
  function patchCardInventory(cardBody, fishList, stoneList, totemList) {
    ...
    if (entry) {
      ...
      entry.lastStoneList = Array.isArray(stoneList) ? stoneList.map((item) => {
        const prev = (entry.lastStoneList || []).find((row) => stoneCardKey(row) === stoneCardKey(item));
        return preferFishCardImageFields(prev, item);
      }) : [];
      entry.lastTotemList = Array.isArray(totemList) ? totemList.map((item) => {
        const prev = (entry.lastTotemList || []).find((row) => totemCardKey(row) === totemCardKey(item));
        return preferFishCardImageFields(prev, item);
      }) : [];
    }
  ...
  function isTrackerBackedImageItem(item) {
    ...
    return src === 'tracker_lua_game_asset'
      || src === 'gameitemdb_icon'
      || src === 'inventory_asset_auto'
      || src === 'manual_override'
      || src === 'totem_manual_asset'
      || src === 'stone_manual_asset'
      || src === 'game_fish_icon_catalog';
  }
  function cardImageSourceRank(item) {
    if (!item) return 0;
    const src = item.imageSource || item.iconSource || item.dataImageSource;
    if (src === 'manual_override') return 5;
    if (src === 'totem_manual_asset' || src === 'stone_manual_asset') return 4;
    if (src === 'inventory_asset_auto') return 3;
    if (src === 'tracker_lua_game_asset' || src === 'gameitemdb_icon') return 2;
    if (src === 'game_fish_icon_catalog') return 1;
    return isUsableImageUrl(item.imageUrl) && !isPlaceholderImageUrl(item.imageUrl) ? 1 : 0;
  }
  function preferFishCardImageFields(existing, incoming) {
    const base = { ...(existing || {}), ...(incoming || {}) };
    const existingUrlOk = isUsableImageUrl(existing && existing.imageUrl)
      && !isPlaceholderImageUrl(existing && existing.imageUrl);
    const incomingUrlOk = isUsableImageUrl(incoming && incoming.imageUrl)
      && !isPlaceholderImageUrl(incoming && incoming.imageUrl);
    const existingRank = cardImageSourceRank(existing);
    const incomingRank = cardImageSourceRank(incoming);
    if (incomingUrlOk && incomingRank >= existingRank) return base;
    if (existingUrlOk && existingRank > incomingRank) {
      return {
        ...base,
        ...
        imageResolver: existing.imageResolver || base.imageResolver,
        manualImageOverride: existing.manualImageOverride || base.manualImageOverride,
      };
    }
    if (incomingUrlOk) return base;
    if (existingUrlOk) {
      return {
        ...base,
        imageUrl: existing.imageUrl,
  ...
  function isUsableImageUrl(url) {
    ...
    if (u.startsWith('/api/fishit-tracker/assets/manual/')) return true;
    if (u.startsWith('/api/fishit-tracker/assets/item/')) return true;
...
<div class="card-body" data-card-body></div>`;
  ...
  async function removeTracker(key) {
    ...
    const inTrackers = trackers.has(normalizedKey);
    const inCache = trackedAccountsCache.some((acct) => acct.key === normalizedKey);
    if (!inTrackers && !inCache) return;
    const removed = await persistTrackerRemove(normalizedKey);
    if (!removed) {
  ...
  function bindRemoveMenu() {
    ...
    removeMenuBtn.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
      toggleRemoveMenu().catch((err) => {
        console.error('[inventory] remove menu open failed', err);
      });
    });
    removeMenuBtn.addEventListener('touchend', (e) => {
      e.stopPropagation();
    }, { passive: true });
    ...
    if (!removeMenuDocumentBound) {
      removeMenuDocumentBound = true;
      document.addEventListener('click', (e) => {
        if (Date.now() < removeMenuIgnoreCloseUntil) return;
