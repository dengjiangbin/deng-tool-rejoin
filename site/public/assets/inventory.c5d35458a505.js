...
const LS_KEY        = 'fishit_tracked_users';
  ...
  const EN_DASH = '\u2013';
  const EM_DASH = '\u2014';
  const TRACKER_BUILD_MARKER = 'INVENTORY_TEXT_ONLY_P0_2026_06_19';
  try {
  ...
  function formatInventoryUploadLabel(entry) {
    if (!entry) return '';
    const ageSec = backendInventoryAgeSeconds(entry);
    if (ageSec == null || !Number.isFinite(ageSec)) {
      return 'Inventory waiting for upload';
    }
    const ageText = formatAgeAgoSeconds(Math.max(1, Math.floor(ageSec)));
    if (!ageText) return 'Inventory waiting for upload';
    if (ageSec <= SYNC_LIVE_MAX_SEC) return `Inventory updated ${ageText}`;
    return `Inventory stale ${ageText}`;
  }
  ...
  function patchInventoryUploadIndicatorDom(root, entry) {
    ...
    if (wrapEl && wrapEl.classList.contains('inventory-upload-indicator')) {
      wrapEl.classList.remove('is-live', 'is-stale');
      if (!wrapEl.classList.contains('is-neutral')) wrapEl.classList.add('is-neutral');
      const title = label || 'Inventory upload status';
