...
const LS_KEY        = 'fishit_tracked_users';
  ...
  function entryHasInventoryRows(data) {
    ...
    const rows = countArr(data.fishItems) + countArr(data.publicItems) + countArr(data.publicFishItems)
      ...
      + countArr(data.lastGoodPublicTotemItems);
    return rows > 0;
  }
  function hasRenderableTrackerData(snapshot) {
    return Boolean(snapshot && (
      hasLeaderstats(snapshot)
      || entryHasInventoryRows(snapshot)
      || (Array.isArray(snapshot.totemItems) && snapshot.totemItems.length > 0)
    ));
  }
  function shouldReplaceCurrentSnapshot(candidate, current) {
    ...
    const candInv = entryHasInventoryRows(candidate);
    const curInv = entryHasInventoryRows(current);
    if (!candInv && curInv) return false;
    if (candInv && !curInv) return true;
    if (candInv && curInv) return isNewerSnapshot(candidate, current);
  ...
  function buildLiveSnapshotFromPayload(entry, data, pollAt) {
    ...
    return {
      pollAt,
      payloadAt: (snapData && (snapData.playerStatsUpdatedAt || snapData.lastInventoryAt || snapData.updatedAt)) || pollAt,
  ...
  function renderAccountsTable() {
    ...
    if (accountsTableBodyEl) {
      if (!rows.length) {
        accountsTableBodyEl.innerHTML = '<tr><td colspan="7" class="accounts-table__empty">No matching accounts yet.</td></tr>';
      } else {
