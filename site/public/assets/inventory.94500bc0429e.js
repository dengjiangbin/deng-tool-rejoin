...
const LS_KEY        = 'fishit_tracked_users';
  ...
  function isTrackerOnline(entry) {
    ...
    if (data && (data.connectionStatus === 'stale' || data.connectionStatus === 'error' || data.connectionStatus === 'offline')) {
      return false;
    }
    const ts = statsSyncTimestamp(data) || entry.lastServerSyncAt;
    return syncFreshnessFromTimestamp(ts) === 'live';
  }
  ...
  function entryConnectionFreshness(entry) {
    ...
    const data = entry.lastData;
    if (data && data.connectionStatus === 'live') return 'live';
    if (data && data.connectionStatus === 'stale') return 'stale';
    if (data && (data.connectionStatus === 'error' || data.connectionStatus === 'offline')) return 'dead';
    const serverTs = statsSyncTimestamp(data) || entry.lastServerSyncAt;
    if (serverTs) return syncFreshnessFromTimestamp(serverTs);
    const hbTs = data && (data.lastHeartbeatAt || data.lastSeenAt);
    if (hbTs && syncAgeSeconds(hbTs) <= SYNC_STALE_MAX_SEC) return 'stale';
    return 'dead';
  }
