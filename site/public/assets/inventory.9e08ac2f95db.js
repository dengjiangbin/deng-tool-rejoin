...
const LS_KEY        = 'fishit_tracked_users';
  ...
  function patchFishCardDom(card, item, opts) {
    ...
    patchCardImage(img, item, imgSrc, true);
  ...
  function patchStoneCardDom(card, item, opts) {
    ...
    if (imgSrc) {
      ...
      patchCardImage(img, item, imgSrc, false);
  ...
  function entrySuccessUploadAt(entry) {
    const data = entry && entry.lastData;
  ...
  function formatTableSyncStatusText(entry) {
    if (!entry) return '';
    if (isEntryStatusGreen(entry)) return formatMinimalSyncDuration(entrySuccessUploadAt(entry));
    return formatMinimalSyncDuration(entryRedSince(entry));
  }
  ...
  function displayCoinsStat(stats) {
    ...
    if (!s || s.source === 'missing') return EMPTY_STAT;
    if (s.coins != null) {
  ...
  function displayRarestFishStat(stats) {
    const s = displayableEntryPlayerStats(stats);
    if (!s || s.source === 'missing' || !s.rarestFishChance) return EMPTY_STAT;
  ...
  <div class="accounts-mobile-card__grid accounts-mobile-card__grid--stats">
    ...
    <div class="accounts-mobile-card__row col-total-caught" data-col="total-caught"><span class="accounts-mobile-card__row-label">Caught</span><span class="accounts-mobile-card__row-value total-caught-value${displayTotalCaughtStat(stats) === EMPTY_STAT ? ' is-muted' : ''}">${escHtml(displayTotalCaughtStat(stats))}</span></div>
  ...
  <td class="accounts-table__stat col-caught col-total-caught total-caught-value${displayTotalCaughtStat(stats) === EMPTY_STAT ? ' is-muted' : ''}" data-col="total-caught">${escHtml(displayTotalCaughtStat(stats))}</td>
  ...
  function patchItemsGrid(container, items, opts) {
    ...
    items.forEach((item, idx) => {
      ...
      let card = existing.get(key);
      if (!card) {
        card = buildItemCardElement(item, { ...itemOpts, keyFn });
        placeGridCardAtIndex(grid, card, idx);
      } else {
        patchItemCardElement(card, item, { ...itemOpts, keyFn });
        placeGridCardAtIndex(grid, card, idx);
      }
    });
