...
const LS_KEY        = 'fishit_tracked_users';
  ...
  function maybeResetSectionTimers(entry) {
    ...
    if (hasRenderableTrackerData(data)) {
      ...
      if (sig !== entry._trackerDisplaySig) {
        ...
        markEntryFrontendRefreshed(entry);
      }
    }
    if (payloadHasLeaderstats(data)) {
      ...
      if (sig !== entry._leaderstatsDisplaySig) {
        ...
        markEntryLeaderstatsRefreshed(entry);
      }
    }
    if (entryHasInventoryRows(data)) {
      ...
      if (sig !== entry._inventoryDisplaySig) {
        ...
        markEntryInventoryRefreshed(entry);
      }
    }
  }
  ...
  function seedOfflineTimersFromBackend(entry) {
    if (!entry || entry._offlineTimersSeeded) return;
    if (isTrackerAccountOnline(entry, Date.now())) return;
    const now = Date.now();
    let seeded = false;
    const seed = (ageSecs, field) => {
      const n = Number(ageSecs);
      if (!Number.isFinite(n) || n <= 0) return;
      entry[field] = now - Math.round(n * 1000);
      seeded = true;
    };
    seed(backendPresenceAgeSeconds(entry), '_frontendRefreshAt');
    seed(backendStatsAgeSeconds(entry), '_leaderstatsFrontendRefreshAt');
    seed(backendInventoryAgeSeconds(entry), '_inventoryFrontendRefreshAt');
    if (seeded) entry._offlineTimersSeeded = true;
  }
