...
const LS_KEY        = 'fishit_tracked_users';
  ...
  function isRubyGemstoneItem(item) {
    ...
    const name = String(item.name || item.displayName || item.baseFishName || item.Name || '')
      .toLowerCase().replace(/\[[^\]]*\]/g, ' ').replace(/\s+/g, ' ').trim();
    if (!name.includes('ruby')) return false;
    const cat = String(item.category || item.type || item.Category || '').toLowerCase();
    const mut = String(item.mutation || '').toLowerCase();
    const tags = Array.isArray(item.mutationTags) ? item.mutationTags.map((t) => String(t).toLowerCase()) : [];
    const hasGemstoneContext = cat.includes('gemstone') || mut.includes('gemstone')
      || tags.some((t) => t.includes('gemstone')) || name.includes('gemstone');
    if (name === 'ruby mutation gemstone' || name === 'ruby gemstone') return true;
    if (name === 'ruby') return hasGemstoneContext;
    return name.includes('ruby') && hasGemstoneContext;
  }
  ...
  function updateInventoryStats() {
    ...
    const stats = lastValidTrackerSummary
      ? {
        totalAccounts: lastValidTrackerSummary.trackedCount,
        onlineCount: lastValidTrackerSummary.onlineCount,
  ...
  function ftDetailMeta(item, kind) {
    ...
    if (k === 'stone') {
      ...
      tag = String(item.stoneType || item.StoneType || '').trim() || 'Enchant Stone';
    } else if (k === 'totem') {
      rawName = String(item.name || item.displayName || 'Totem');
      tag = String(item.mutation || '').trim() || 'Totem';
    } else {
      rawName = cardTitle(item);
      const muts = (typeof publicMutationBadges === 'function') ? publicMutationBadges(item) : [];
      tag = (item.mutation && String(item.mutation).trim())
        || (muts && muts[0])
  ...
  let ftDetailModalEl = null;
  function ensureFtDetailModal() {
    if (ftDetailModalEl) return ftDetailModalEl;
    const overlay = document.createElement('div');
    overlay.className = 'ft-detail-overlay';
    overlay.id = 'ftDetailModal';
    overlay.hidden = true;
    overlay.setAttribute('aria-hidden', 'true');
    overlay.innerHTML = [
      '<div class="ft-detail-dialog" role="dialog" aria-modal="true" aria-labelledby="ftDetailName">',
      ...
      '      <div class="ft-detail-name" id="ftDetailName" data-ft-detail-name></div>',
      '      <div class="ft-detail-meta" data-ft-detail-meta></div>',
      '    </div>',
      '  </div>',
      '  <div class="ft-detail-owners-title">Owned by</div>',
      '  <div class="ft-detail-owners" data-ft-detail-owners></div>',
      '</div>',
    ].join('');
    document.body.appendChild(overlay);
    overlay.addEventListener('click', (e) => {
      if (e.target === overlay || e.target.closest('.ft-detail-close')) closeFtDetailModal();
    });
    ftDetailModalEl = overlay;
    return overlay;
  }
  function closeFtDetailModal() {
    if (!ftDetailModalEl) return;
    ftDetailModalEl.hidden = true;
    ftDetailModalEl.setAttribute('aria-hidden', 'true');
    document.body.style.overflow = '';
  }
  function openFtDetailModal(item, kind) {
    if (!item) return;
    const overlay = ensureFtDetailModal();
    const meta = ftDetailMeta(item, kind);
    const total = Math.max(0, Math.floor(Number(resolveItemAmount(item)) || 0));
    const owners = ftDetailOwnerRows(item);
    const iconEl = overlay.querySelector('[data-ft-detail-icon]');
    const tagEl = overlay.querySelector('[data-ft-detail-tag]');
    const nameEl = overlay.querySelector('[data-ft-detail-name]');
    const metaEl = overlay.querySelector('[data-ft-detail-meta]');
    const ownersEl = overlay.querySelector('[data-ft-detail-owners]');
    ...
    if (tagEl) {
      if (meta.tag) { tagEl.textContent = `[${meta.tag}]`; tagEl.hidden = false; }
      else { tagEl.textContent = ''; tagEl.hidden = true; }
    }
    if (nameEl) nameEl.textContent = meta.name;
    if (metaEl) {
      const bits = [];
      if (meta.rarity && meta.rarity !== 'Unknown') bits.push(escHtml(meta.rarity));
      bits.push(`Total x${formatQuantity(total)}`);
      metaEl.innerHTML = bits.map((b) => `<span class="ft-detail-chip">${b}</span>`).join('');
    }
    if (ownersEl) {
      if (owners.length) {
        ownersEl.innerHTML = owners.map((row) => (
          `<div class="ft-detail-owner"><span class="ft-detail-owner__name">${escHtml(row.username)}</span><span class="ft-detail-owner__qty">x${formatQuantity(row.amount)}</span></div>`
        )).join('');
      } else {
        ownersEl.innerHTML = '<div class="ft-detail-owner ft-detail-owner--empty">No ownership data</div>';
      }
    }
    overlay.hidden = false;
    overlay.setAttribute('aria-hidden', 'false');
    document.body.style.overflow = 'hidden';
    const closeBtn = overlay.querySelector('.ft-detail-close');
    if (closeBtn) setTimeout(() => closeBtn.focus(), 0);
  }
  function bindFtDetailModal() {
    document.addEventListener('click', (e) => {
      const card = e.target.closest && e.target.closest('.ft-card--interactive');
      if (!card || !card.__ftItem) return;
      openFtDetailModal(card.__ftItem, card.__ftKind);
    });
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && ftDetailModalEl && !ftDetailModalEl.hidden) {
        e.preventDefault();
        closeFtDetailModal();
        return;
      }
      if ((e.key === 'Enter' || e.key === ' ') && e.target && e.target.classList && e.target.classList.contains('ft-card--interactive') && e.target.__ftItem) {
        e.preventDefault();
        openFtDetailModal(e.target.__ftItem, e.target.__ftKind);
      }
    });
  }
...
<div class="card-body" data-card-body></div>`;
  ...
  function initInventoryUi() {
    ...
    safeBind('card detail modal', bindFtDetailModal);
