...
const LS_KEY        = 'fishit_tracked_users';
  ...
  function applyAccountStatusPayload(payload) {
    ...
    trackers.forEach((entry, key) => {
      ...
      if (st) {
        ...
        if (payloadHasLeaderstats(st)) markEntryLeaderstatsRefreshed(entry);
  ...
  function applyInventoryPollPayload(entry, key, data) {
    if (hasRenderableTrackerData(data)) markEntryFrontendRefreshed(entry);
    if (payloadHasLeaderstats(data)) markEntryLeaderstatsRefreshed(entry);
    if (entryHasInventoryRows(data)) markEntryInventoryRefreshed(entry);
  ...
  function rubyGemstoneCountForItem(item) {
    ...
    if (list && list.length) {
      ...
      for (const inst of list) {
        const mut = inst && (inst.mutationName || inst.mutation || inst.metadataMutation);
        if (isRubyGemstoneMutationName(mut)) {
          n += Math.max(1, Math.floor(Number(inst.quantity) || 1));
        }
      }
      if (n > 0) return n;
    }
