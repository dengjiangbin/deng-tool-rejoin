...
const TRACKER_RUNIC_STONE_IMAGE = "/public/assets/tracker-top-grid/runic-stone.png";
...
function getTrackerItemImage(nameOrItem) {
  ...
  const EN_DASH = '\u2013';
  const EM_DASH = '\u2014';
  const TRACKER_BUILD_MARKER = 'TRACKER_SERVERNOW_TIMER_502_FIX_2026_06_25';
  try {
  ...
  function updateTrackerServerClock(serverNow) {
    const serverMs = parseServerTimeMs(serverNow);
    if (serverMs == null) return null;
    trackerServerClockOffsetMs = serverMs - Date.now();
    accountStatusServerNowMs = serverMs;
    return serverMs;
  }
  function correctedClientNowMs() {
    return Date.now() + trackerServerClockOffsetMs;
  }
  ...
  function mergeUploadStatusOntoEntry(entry, statusRow, serverNow) {
    ...
    entry._uploadStatusFetchedAtMs = updateTrackerServerClock(serverNow) || correctedClientNowMs();
  }
  function liveDriftedSeconds(st, secondsField, fetchedAtMs) {
    ...
    const drift = Math.floor((correctedClientNowMs() - fetchedAtMs) / 1000);
  ...
  function applyAccountStatusPayload(payload) {
    ...
    accountStatusServerNowMs = updateTrackerServerClock(payload.serverNow) || correctedClientNowMs();
    const byKey = new Map();
    ...
    trackers.forEach((entry, key) => {
      ...
      if (st) {
        ...
        mergeLeaderstatsLaneAuthFromAccountStatus(entry, st);
        mergeInventoryLaneAuthFromAccountStatus(entry, st);
  ...
  function reconcileEntryPresence(entry, incoming, serverNowMs) {
    ...
    var nowMs = Number(serverNowMs) || correctedClientNowMs();
  ...
  function syncAgeSeconds(timestamp) {
    ...
    try {
      const tsMs = parseServerTimeMs(timestamp);
      if (tsMs == null) return null;
      const secs = Math.floor((correctedClientNowMs() - tsMs) / 1000);
  ...
  function authAgeSecondsFromTs(ts) {
    if (!ts) return null;
    const ms = parseServerTimeMs(ts);
    if (!Number.isFinite(ms)) return null;
    return Math.max(0, Math.floor((correctedClientNowMs() - ms) / 1000));
  }
  ...
  function backendStatsAgeSeconds(entry) {
    ...
    const live = liveSecondsSinceStatsSuccess(entry);
    if (live != null) ages.push(live);
    if (!ages.length) return null;
    return Math.min(...ages);
  }
  function mergeLeaderstatsLaneAuthFromAccountStatus(entry, st) {
    if (!entry || !st) return;
    const nextTs = st.lastRealLeaderstatsAt || st.leaderstatsLastSuccessAt || st.lastStatsUploadAt || null;
    if (!nextTs) return;
    const prev = entry._auth || {};
    const prevTs = prev.lastRealLeaderstatsAt || null;
    const nextMs = Date.parse(nextTs);
    const prevMs = prevTs ? Date.parse(prevTs) : NaN;
    if (!Number.isFinite(nextMs)) return;
    if (Number.isFinite(prevMs) && nextMs <= prevMs) return;
    entry._auth = {
      ...prev,
      lastRealLeaderstatsAt: nextTs,
      leaderstatsRevision: st.leaderstatsRevision != null
        ? Number(st.leaderstatsRevision)
        : prev.leaderstatsRevision,
    };
  }
  function mergeInventoryLaneAuthFromAccountStatus(entry, st) {
    if (!entry || !st) return;
    const nextTs = st.lastRealInventoryAt || st.inventoryLastRealUploadAt || st.lastInventoryAt || null;
    if (!nextTs) return;
    const prev = entry._auth || {};
    const prevTs = prev.lastRealInventoryAt || null;
    const nextMs = Date.parse(nextTs);
    const prevMs = prevTs ? Date.parse(prevTs) : NaN;
    if (!Number.isFinite(nextMs)) return;
    if (Number.isFinite(prevMs) && nextMs <= prevMs) return;
    const nextRev = st.inventoryRevision != null ? Number(st.inventoryRevision) : null;
    const prevRev = prev.inventoryRevision != null ? Number(prev.inventoryRevision) : null;
    if (Number.isFinite(prevRev) && Number.isFinite(nextRev) && nextRev <= prevRev && nextMs <= prevMs) return;
    entry._auth = {
      ...prev,
      lastRealInventoryAt: nextTs,
      inventoryRevision: Number.isFinite(nextRev) ? nextRev : prev.inventoryRevision,
    };
  }
  function inventoryLaneStaleThresholdSeconds() {
    return Number(window.__TRACKER_INVENTORY_STALE_SECS__) > 0
      ? Number(window.__TRACKER_INVENTORY_STALE_SECS__)
      : 300;
  }
  function isStatusFreshInventoryStale(entry) {
    if (!entry) return false;
    if (!isTrackerAccountOnline(entry, Date.now())) return false;
    const statusAge = backendPresenceAgeSeconds(entry);
    const invAge = backendInventoryAgeSeconds(entry);
    if (statusAge == null) return false;
    if (invAge == null) return true;
    return invAge > inventoryLaneStaleThresholdSeconds() && statusAge <= inventoryLaneStaleThresholdSeconds();
  }
  function inventoryLaneStaleHint(entry) {
    if (!isStatusFreshInventoryStale(entry)) return '';
    const invAge = backendInventoryAgeSeconds(entry);
    if (invAge == null) return 'Online — inventory not uploaded yet';
    return 'Online — inventory upload stale';
  }
  ...
  function applyInventoryPollPayload(entry, key, data) {
    ...
    reconcileEntryPresence(entry, data, updateTrackerServerClock(data && data.serverNow) || correctedClientNowMs());
    const pollAt = new Date().toISOString();
  ...
  function patchInventoryUploadIndicatorDom(root, entry) {
    ...
    const staleHint = entry ? inventoryLaneStaleHint(entry) : '';
    const displayLabel = staleHint || label;
    if (textEl && textEl.textContent !== (displayLabel || '')) textEl.textContent = displayLabel || '';
    if (wrapEl && wrapEl.classList.contains('inventory-upload-indicator')) {
      ...
      const title = staleHint ? staleHint : (label || '');
      if (wrapEl.getAttribute('title') !== title) wrapEl.setAttribute('title', title);
      if (staleHint) wrapEl.setAttribute('data-inventory-lane-stale', '1');
      else wrapEl.removeAttribute('data-inventory-lane-stale');
    }
  }
  ...
  function itemImageSrc(item) {
    const trackerOverride = (typeof resolveTrackerItemImageOverride === 'function')
      ? resolveTrackerItemImageOverride(item)
      : null;
    if (trackerOverride) return trackerReadPath(trackerOverride);
  ...
  function isUsableImageUrl(url) {
    ...
    if (u.startsWith('/public/assets/tracker-top-grid/')) return true;
    if (u.startsWith('/public/assets/')) return true;
    return false;
  }
...
<div class="card-body" data-card-body></div>`;
  ...
  function readReadApiPresence(res) {
    ...
    return {
      ...
      serverNow: g('X-DENG-Server-Now') || null,
  ...
  function applyAuthPresence(entry, key, contract) {
    ...
    updateTrackerServerClock(contract.serverNow);
  ...
  async function pollUser(key, opts) {
    ...
    try {
      ...
      updateTrackerServerClock((data && data.serverNow) || (contract && contract.serverNow));
