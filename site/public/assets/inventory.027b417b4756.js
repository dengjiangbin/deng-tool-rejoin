...
const LS_KEY        = 'fishit_tracked_users';
  ...
  function entryHasInventoryRows(data) {
    ...
    if (data.provenEmptyInventory === true) return false;
    const countArr = (v) => (Array.isArray(v) ? v.length : 0);
    return countArr(data.fishItems) + countArr(data.publicItems) + countArr(data.publicFishItems)
      + countArr(data.stoneItems) + countArr(data.totemItems)
      + countArr(data.lastGoodPublicFishItems) + countArr(data.lastGoodPublicStoneItems)
      + countArr(data.lastGoodPublicTotemItems) > 0;
  }
  function mergePreservedInventorySnapshot(previous, incoming) {
    if (!incoming || typeof incoming !== 'object') return previous || null;
    const merged = { ...(previous || {}), ...incoming };
    if (entryHasInventoryRows(incoming) || incoming.provenEmptyInventory === true) return merged;
    if (!entryHasInventoryRows(previous)) return merged;
    for (const key of INVENTORY_PRESERVE_KEYS) {
  ...
  function applyLiveSnapshotToPublicUi(entry, key, data) {
    ...
    if (!snap && !fishList.length && !stoneList.length) return;
  ...
  function ftDetailMutationSlug(mutation) {
    const key = String(mutation || '').toLowerCase().trim();
    if (!key) return '';
    if (key.includes('gold')) return 'gold';
    if (key.includes('gemstone')) return 'gemstone';
    if (key === 'albino') return 'albino';
    if (key === 'stone' || (key.includes('stone') && !key.includes('gemstone'))) return 'stone';
    if (key.includes('sand')) return 'sandy';
    return 'custom';
  }
  function ftDetailMutationThemeVars(mutation, rarityKey) {
    const slug = ftDetailMutationSlug(mutation);
    if (slug && slug !== 'custom') return '';
    const accent = nudgeHexAwayFromRarity(ftMutationColor(mutation), rarityKey, 72);
    const bg = nudgeHexAwayFromRarity(
      rgbToHex(parseHexColor(accent)[0] * 0.22, parseHexColor(accent)[1] * 0.22, parseHexColor(accent)[2] * 0.22),
      rarityKey,
      64,
    );
    const border = nudgeHexAwayFromRarity(accent, rarityKey, 80);
    const text = getReadableTextColor(bg);
    const pillBg = nudgeHexAwayFromRarity(accent, rarityKey, 72);
    const pillText = getReadableTextColor(pillBg);
    return `--tdf-bg:${bg};--tdf-border:${border};--tdf-text:${text};--tdf-pill-bg:${pillBg};--tdf-pill-text:${pillText};`;
  }
  function sortFishDetailInstances(a, b) {
    ...
    if (aHasMutation && bHasMutation) {
      const rankA = getMutationSortRank(aMutation);
      const rankB = getMutationSortRank(bMutation);
      if (rankA !== rankB) return rankA - rankB;
      if (rankA >= 1000 && rankB >= 1000) {
        const alpha = aMutation.localeCompare(bMutation);
    ...
    const aNum = Number.isFinite(aWeight) ? aWeight : -1;
    const bNum = Number.isFinite(bWeight) ? bWeight : -1;
  ...
  function renderFishInstanceCard(card) {
    ...
    const mut = realMut
      ...
      : '';
    return `<div class="${escHtml(classes.join(' '))}"${styleAttr}>${img}<div class="tracker-detail-fish-card__body">${weight}${mut}<div class="tracker-detail-fish-name">${escHtml(card.name)}</div><div class="tracker-detail-fish-owner">${escHtml(card.owner || '-')}</div></div></div>`;
  }
