...
def build_heartbeat_tracker_lua(
    ...
    *,
    ...
    return (
        "-- Deng Local Heartbeat (Robust Detection Logic)\n"
        "task.spawn(function()\n"
        "    -- 1. Wait for engine load\n"
        "    if not game:IsLoaded() then\n"
        "        game.Loaded:Wait()\n"
        "    end\n"
        "\n"
        "    -- 2. Wait for LocalPlayer to spawn\n"
        "    local Players = game:GetService(\"Players\")\n"
        "    while not Players.LocalPlayer do\n"
        "        task.wait(1)\n"
        "    end\n"
        "\n"
        "    -- 3. Ping local Termux watchdog\n"
        ...
        "end)\n"
        "\n"
        "-- Chain-load user primary script\n"
        "pcall(function()\n"
