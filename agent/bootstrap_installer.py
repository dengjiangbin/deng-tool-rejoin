    ...
    *,
    ...
    if channel.strip().lower() in _dev_channels:
        test_bypass_part = (
            ...
            'ok "TEST LICENSE BYPASS ACTIVE (test channel - no key required)"\n'
        )
    banner_part = ""
    if banner_lines:
    ...
    pre_heredoc = (
        'C="";B="";Y="";G="";R="";X=""\n'
        'if [ -t 1 ]; then C="\\033[1;96m";B="\\033[1;94m";Y="\\033[1;93m";G="\\033[1;92m";R="\\033[1;91m";X="\\033[0m"; fi\n'
        'sep="=============================="; dash="------------------------------"\n'
        'info(){ printf "%b[*]%b %s\\n" "$Y" "$X" "$1"; }\n'
        'ok(){ printf "%b[+]%b %s\\n" "$G" "$X" "$1"; }\n'
        'fail(){ printf "%b[!]%b %s\\n" "$R" "$X" "$1" >&2; exit 1; }\n'
        ...
        'command -v python3 >/dev/null 2>&1 || fail "Install python first: pkg install -y python"\n'
        ...
        'case "$p" in "$u"/api/download/package/*) ;; *) fail "Download setup failed." ;; esac\n'
        'info "Downloading protected package..."\n'
        'curl -fsSL -H "Cache-Control: no-cache" -H "Pragma: no-cache" -A "deng-rejoin-installer/2.0" "$p" -o "$t" || fail "Package download failed."\n'
        ...
        'tar -xzf "$t" -C "$h" || fail "Could not extract package."\n'
        'find "$h" -depth -name __pycache__ -type d -exec rm -rf {} + 2>/dev/null || true\n'
        'find "$h" -name "*.pyc" 2>/dev/null -exec rm -f {} + || true\n'
        '[ -f "$h/agent/deng_tool_rejoin.py" ] || fail "Install error: runtime missing."\n'
        '[ -f "$h/agent/.deng_runtime.bin" ] || fail "Install error: protected runtime missing."\n'
        '[ -f "$h/BUILD-INFO.json" ] || fail "Install error: BUILD-INFO.json missing."\n'
        '[ -f "$h/RELEASE-MANIFEST.json" ] || fail "Install error: RELEASE-MANIFEST.json missing."\n'
    ...
    post_heredoc = (
        ...
        "chmod +x \"$BIN/deng-rejoin\" || { echo \"chmod failed: $BIN/deng-rejoin\" >&2; exit 1; }\n"
        "hash -r 2>/dev/null || true\n"
        '[ -s "$BIN/deng-rejoin" ] || fail "Failed to create deng-rejoin wrapper."\n'
        '[ -x "$BIN/deng-rejoin" ] || fail "Failed: wrapper not executable."\n'
        '[ -f "$h/agent/deng_tool_rejoin.py" ] || fail "Failed: runtime missing."\n'
